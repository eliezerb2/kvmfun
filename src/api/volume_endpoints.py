...
router = APIRouter(
    prefix=config.VOLUME_ROUTER_PREFIX,
    tags=["volume"],
    responses={
...
async def create_volume_endpoint(request: CreateVolumeRequest, conn: libvirt.virConnect = Depends(get_connection_dependency)):
    """
...
async def delete_volume_endpoint(request: BaseVolumeRequest = Depends(), conn: libvirt.virConnect = Depends(get_connection_dependency)):
    ...
    """
    logger.info(f"Disk volume deletion request - Pool: {request.pool_name}, Name: {request.volume_name}")
    try:
        delete_volume(conn, request.pool_name, request.volume_name)
